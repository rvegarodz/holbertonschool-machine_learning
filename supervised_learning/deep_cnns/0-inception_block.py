...
def inception_block(A_prev, filters):
    ...
    convolution_1 = (K.layers.Conv2D(filters=F1,
                                     kernel_size=1,
                                     padding='same',
                                     activation=activation,
                                     kernel_initializer=init)(A_prev))
    convolution_2P = (K.layers.Conv2D(filters=F3R,
                                      kernel_size=1,
                                      padding='same',
                                      activation=activation,
                                      kernel_initializer=init)(A_prev))
    convolution_2 = (K.layers.Conv2D(filters=F3,
                                     kernel_size=3,
                                     padding='same',
                                     activation=activation,
                                     kernel_initializer=init)(convolution_2P))
    convolution_3P = (K.layers.Conv2D(filters=F5R,
                                      kernel_size=1,
                                      padding='same',
                                      activation=activation,
                                      kernel_initializer=init)(A_prev))
    ...
    mid_layer = (K.layers.concatenate([convolution_1,
                                       convolution_2,
                                       convolution_3,
                                       layer_poolP]))
    return mid_layer
